...
| 生气专用 | `PUCKER` | -1～1 | 负端未使用 | 1 看起来脸更圆 | 不设独立控制；仅“生气”预设从 0 平滑到 1，关闭时平滑退回。 |
...
- v0.1.42 的自动参数混合验证可按“菜单→点单→双餐盘→生气→wink”顺序运行；导出诊断 `maid_parameter_mixer.samples` 保存每个阶段的预设权重和最终参数值，辅助检查叠加、互斥与退场。小形态三参数的最终顶点围绕领口 `Part27` 的脖子固定点补偿平移，诊断另存 `small_neck_anchor` 与逐帧修正；这不是将三参数值重写为新动作。
- v0.1.41 的女仆预设按原装 `.exp3.json` 的 Add 参数目标在 0.38 秒内平滑渐入/渐出，手动关闭也退回当前下层值；菜单换点单时两个目标同时过渡，公共 `Param79=1` 不先归零。原装 `2餐盘左` 的 `Param119` 与 `2餐盘右` 的 `Param115` 可同时开启，后一只餐盘也平滑加入；选择其他 `2` 组动作时双餐盘一起退出。
- `1生气` 保留原装 `Param148`，附加 `PUCKER` 从 0 到 1，并提取原装 `待机动画.motion3.json` 的 `Param149/150` 循环，只在显式生气期间播放两侧冒气。变小时不启动完整生气预设，按已识别的“不高兴”端 `ParamMouthForm=-1` 过渡嘴型，仍按原设计默认附带 `2插手`；可在变小期间另外点击“生气”。
- 新增三种左眼预设：wink、wink＋吐舌、比耶＋wink＋吐舌。分别控制左眼开闭与微笑、`OUT` 和原装 `2比耶`，同样可切换及平滑退出。吐舌本身按短舌观察目标直达 1；预设层整体权重随切换平滑变化。白袜/黑袜 `Param41` 和变小/变大 `Param154/157/156` 也走同一过渡。
...
真机重点看：菜单→点单不中断、左右餐盘叠加、全预设关闭的退场、变大与黑袜回退、生气两侧冒气和小形态嘴型，以及三档 wink 与自然眨眼/耳鳍联动。JEV 调用策略应在 AI 伴侣的单 DeepSeek / DeepSeek+Gemini 现有请求编排中设计，避免在测试仓增设一条独立的每帧 API 路径。
